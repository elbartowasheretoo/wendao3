import { removeFederation, getCachedDocumentNodeFromSchema, oldVisit } from '@graphql-codegen/plugin-helpers';
import { Kind, isEnumType, GraphQLObjectType, visit, buildASTSchema, TypeInfo, visitWithTypeInfo, getNamedType, isIntrospectionType, parse, printIntrospectionSchema, isObjectType } from 'graphql';
import { OperationVariablesToObject, normalizeAvoidOptionals, BaseTypesVisitor, getConfigValue, DeclarationBlock, indent, transformComment, wrapWithSingleQuotes } from '@graphql-codegen/visitor-plugin-common';
import autoBind from 'auto-bind';
import 'path';

class TypeScriptOperationVariablesToObject extends OperationVariablesToObject {
    constructor(_scalars, _convertName, _avoidOptionals, _immutableTypes, _namespacedImportName = null, _enumNames = [], _enumPrefix = true, _enumValues = {}, _applyCoercion = false, _directiveArgumentAndInputFieldMappings = {}) {
...
class TsVisitor extends BaseTypesVisitor {
    constructor(schema, pluginConfig, additionalConfig = {}) {
        ...
        const enumNames = Object.values(schema.getTypeMap())
            .filter(isEnumType)
            .map(type => type.name);
        this.setArgumentsTransformer(new TypeScriptOperationVariablesToObject(this.scalars, this.convertName, this.config.avoidOptionals, this.config.immutableTypes, null, enumNames, pluginConfig.enumPrefix, this.config.enumValues, false, this.config.directiveArgumentAndInputFieldMappings));
    ...
    InputValueDefinition(node, key, parent) {
        ...
        return (comment +
            indent(`${this.config.immutableTypes ? 'readonly ' : ''}${node.name}${addOptionalSign ? '?' : ''}: ${type}${this.getPunctuation(declarationKind)}`));
    }
...
export { EXACT_SIGNATURE, MAKE_MAYBE_SIGNATURE, MAKE_OPTIONAL_SIGNATURE, TsIntrospectionVisitor, TsVisitor, TypeScriptOperationVariablesToObject, includeIntrospectionDefinitions, plugin };
